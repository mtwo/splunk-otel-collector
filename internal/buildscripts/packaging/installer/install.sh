...
parse_args_and_install() {
  ...
  if [ "$mode" = "agent" ]; then
    if [ -f "$agent_config_path" ]; then
      # use the agent config if installing with fluentd
    ...
    if [ -f "$gateway_config_path" ]; then
      # use the gateway config if installing without fluentd
      collector_config_path="$gateway_config_path"
    elif [ -f "$old_config_path" ]; then
      # use the old config if the installed package does not include the new gateway config
      collector_config_path="$old_config_path"
    fi
  fi
